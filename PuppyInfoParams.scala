...
val df = sqlContext.read.format("com.databricks.spark.csv").option("header", "true").load("datasets/2fdataset.csv")
...
val hashingTF1 = new HashingTF().setInputCol("filtered4").setOutputCol("rawfeatures4").setNumFeatures(20)
...
val hashingTF2 = new HashingTF().setInputCol("filtered5").setOutputCol("rawfeatures5").setNumFeatures(20)
